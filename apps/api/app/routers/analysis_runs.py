...
Lists analysis runs for a workspace.  The endpoint is thin: it
...
@router.get(
    ...
    summary="List analysis runs for a workspace",
    description=(
        "Returns a paginated list of analysis runs scoped to a workspace, "
...
def list_analysis_runs(
    workspace_id: UUID = Query(..., description="Workspace to list runs for."),
    limit: int = Query(50, ge=1, le=200, description="Max rows to return."),
    offset: int = Query(0, ge=0, description="Pagination offset."),
    db: Session = Depends(get_db),
    current_user: CurrentUser = Depends(get_current_user),
) -> list[AnalysisRunSummary]:
    """List analysis runs for a workspace with membership enforcement."""
    ...
    if not ws_repo.is_member(workspace_id, current_user.user_id):
        raise HTTPException(
    ...
    rows = run_repo.list_for_workspace(workspace_id, limit=limit, offset=offset)
